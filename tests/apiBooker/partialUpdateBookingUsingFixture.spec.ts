...
test(`partial update booking`, async ({ request, apiToken, bookingID }) => {
  ...
  //A: Action: call request to partial update booking
  ...
  const partialUpdateBookingIdRes = await request.put(
    ...
    {
      data: {
        firstname: "FN_Buddy_updated_partial",
        lastname: "LN_Buddy_updated_partial",
        totalprice: 122,
        depositpaid: true,
        bookingdates: { checkin: "2025-07-10", checkout: "2025-07-12" },
        // additionalneeds: "Dinner",
  ...
  //A: Assert: assert value partially
  expect(partialUpdateBookingIdRes.status()).toBe(200);
  const partialResponseBody = await partialUpdateBookingIdRes.json();
  console.log(partialResponseBody);
  expect(partialResponseBody).toEqual({
    firstname: "FN_Buddy_updated_partial",
    lastname: "LN_Buddy_updated_partial",
    totalprice: 122,
    depositpaid: true,
